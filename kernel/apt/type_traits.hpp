...
namespace apt {
  namespace impl {
    ...
    template < typename T > struct is_ref : False {};
    template < typename T > struct is_ref<T&> : True {};
    template < typename T > struct is_ref<T&&> : True {};

    template < typename T > struct is_const : False {};
    template < typename T > struct is_const<const T> : True {};
    // TODO std::is_const_v<T> is false if T is a reference
    template < typename T > struct is_const<const T&> : False {};
    template < typename T > struct is_const<const T&&> : False {};

  }
  template < typename T > inline constexpr auto is_ref_v = impl::is_ref<T>::value;
  template < typename T > inline constexpr auto is_const_v = impl::is_const<T>::value;
  ...
  template < typename T > using remove_ref_t = typename impl::remove_ref<T>::type;
  template < typename T > using remove_cv_t = typename impl::remove_cv<T>::type;

  namespace impl {
    template < bool Cond, typename True, typename False >
    struct cond { using type = True; };

    template < typename True, typename False >
    struct cond< false, True, False > { using type = False; };
  }
  template < bool Cond, typename True, typename False >

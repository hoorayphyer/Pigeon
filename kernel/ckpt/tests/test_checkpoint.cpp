...
SCENARIO("Test save_checkpoint", "[ckpt]") {
  ...
  for (int i = 0; i < num_ens; ++i) ens[i].rank.resize(ens_nprocs[i]);
        ...
        for (auto edir : sf.toc_dir()) {
          if (edir.find("ensemble") == 0) {
            ...
            ens_uncovered.insert(l);
            {
              ...
              // NOTE cartesian rank is the same as ensemble label. TODO
              // cartesian coordinates is not tested
            }
              ...
              if (rdir.find("rank") == 0) {
                ...
                if (r == 0) {
                  ...
                  for (int i = 0; i < DGrid; ++i)
                    field_size *= (2 * guard + bulk_dims[i]);
                  for (int i = 0; i < 3; ++i) {
...
SCENARIO("Test load_checkpoint", "[mpi][ckpt]") {
  ...
  for (auto sp : properties) {
    auto [Nptc_bef, Nptc_aft, count_matched, Nremain_bef, Nremain_aft] =
        debug::compare_particles(std::move(ptcs_bef[sp]),
                                 std::move(ptcs_aft[sp]), mpi::world);
  }

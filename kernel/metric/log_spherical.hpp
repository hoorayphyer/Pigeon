...
struct LogSpherical {
  ...
  static inline T h(T logr, T theta, T = 0.0) noexcept {
    ...
    if constexpr (N == 2)
      return std::exp(logr) * std::sin(theta);
    else
      return std::exp(logr);
  }
  ...
  static inline T hh(T logr, T theta, T = 0.0) noexcept {
    ...
    if constexpr (N == 2)
      ...
      return std::exp(2.0 * logr) * std::sin(theta);
  }
...
struct FH {
  ...
  constexpr T operator()(int shift) const noexcept {
    return *(&f + shift * s[I]) *
           Metric::template h<Fcomp>(lnr + (0 == I) * shift * g[0].delta(),
                                     theta + (1 == I) * shift * g[1].delta());
  }
};

// NOTE all derivatives are second order
template <int DGrid, typename T, int Fcomp, int I, offset_t f_ofs>
constexpr T diff(const T& f, T lnr, T theta, const apt::Grid<T, DGrid>& g,
                 const apt::Index<DGrid + 1>& s) noexcept {
...
constexpr T (*Diff(int Fcomp,
                   int Icoord))(const T& f, T lnr, T theta,
                                const apt::Grid<T, DGrid>& g,
                                const apt::Index<DGrid + 1>& s) noexcept {
  if (0 == Fcomp) {
...
constexpr T (*HHk(int i))(T, T, T) noexcept {
  ...
  switch (i) {
    case 0:
      return Metric::template hh<0>;
    case 1:
      return Metric::template hh<1>;
    case 2:
      return Metric::template hh<2>;
  }
  return nullptr;
}
...
template <int DGrid, typename T, int Fcomp, int I>
constexpr T diff_1sided_from_right(const T& f, T lnr, T theta,
                                   const apt::Grid<T, DGrid>& g,
                                   const apt::Index<DGrid + 1>& s) noexcept {
  if constexpr (I >= DGrid) return 0.0;
  FH<DGrid, T, Fcomp, I> fh{f, lnr, theta, g, s};
...
constexpr T diff_axis_Ephi_theta(const T& f, T lnr, T theta,
                                 const apt::Grid<T, DGrid>& g,
                                 const apt::Index<DGrid + 1>& s) noexcept {
  ...
  if constexpr (IsUpper)
    return 2 * (*(&f - s[I])) * std::exp(-lnr) /
           std::sin(theta);  // NOTE theta here is larger than pi, and this
                             // formula is correct.
  else
    return 2 * f * std::exp(-lnr) / std::sin(theta);
}
...
template <typename T>
template <class X, class P>
auto LogSpherical<T>::geodesic_move(X& x, P& p, T dt,
                                    bool is_massive) noexcept {
  ...
  for (int i = 0; i < P::NDim; ++i) dx[i] = p[i] * dt;
  ...
  {  // compute final coordiantes. In this section, x remains untouched
    ...
    dt = dx[0] * sin +
         dx[1] * cos;       // NOTE now dt stores an important determinant
    is_massive = (dt < 0);  // NOTE is_massive now stores whether axis crossing
                            // happened when displacing in the theta direction

    // rotate dx[0], dx[1] by x[1]. This solves the loss of significance under
    // some circumstances when compiler optimization is strong, which results in
    // either std::sqrt-ing a negative number or std::acos-ing a number larger
    // than 1, causing NAN error
    dx[0] = dx[0] * cos - dx[1] * sin;
    dx[1] = dt;

    dt = (dt == static_cast<T>(0)) ? 0.0
                                   : std::atan(dx[2] / dt) + PI * is_massive;
    dx[2] = std::sqrt(dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2]);
    dx[1] = std::acos(dx[0] / dx[2]);
    std::swap(dt, dx[2]);
    dt = std::log(dt);
    // by now, dt = ln(r_final), dx[0] is free, dx[1] = \theta_final, dx[2] =
    // \phi_final - \phi_init
  }
  {  // rebase velocity to the new position
    // first rotate in r-theta plane to equator. Location is rotated by PI/2 -
    // theta, so velocity components are rotated by theta - PI/2.
    ...
    // then rotate in r-phi plane. Location is rotated by dphi, so velocity
    // components are rotated -dphi
    cos = std::cos(-dx[2]);
    sin = std::sin(-dx[2]);
    dx[0] = p[0];
    ...
    // last rotate in r-theta plane to new location. Location is rotated by
    // theta_new - PI/2, so velocity components are rotated by PI/2 - theta_new
  ...
  {  // in this block we update x
    // NOTE we don't normalize cyclic coordinates such as \phi in spherical.
    // This operation is designated to mesh_shape_interplay
    dx[0] = dt - x[0];
    std::swap(dt, x[0]);

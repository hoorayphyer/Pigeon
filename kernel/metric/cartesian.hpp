...
namespace field {
  ...
  constexpr T diff( const T& f, T , T , const apt::Grid<T,DGrid>& g, const apt::Index<DGrid+1>& s ) noexcept {
  ...
  constexpr T (*Diff( int Fcomp, int Icoord )) (const T& f, T, T, const apt::Grid<T,DGrid>& g, const apt::Index<DGrid+1>& s)  noexcept {
    if ( 0 == Fcomp ) {

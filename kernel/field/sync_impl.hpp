...
// TODOL can we use the most continous part of field for another temporary
// buffer? More generally, can we change the memory layout? Because guard cells
// in other directions are not continous anyway. Those guard cells can be used
...
void sync(Field<T, DField, DGrid>& field, const mpi::CartComm& comm,
  ...
  assert(apt::range::is_margin_uniform(
      domain_range));  // FIXME this implementation requires all margins to be
                       // equal
  // check sub_range \in field.range \in domain_range
  {
    ...
    for (int i = 0; i < DGrid; ++i) {
      bool x = (sub_range[i].far_begin() >= range[i].far_begin() and
                range[i].far_begin() >= domain_range[i].far_begin());
      ordered = (ordered and x);
      x = (sub_range[i].far_end() <= range[i].far_end() and
           range[i].far_end() >= domain_range[i].far_end());
      ordered = (ordered and x);
    }
  ...
  for (int i_grid = 0; i_grid < DGrid; ++i_grid) {
    bool is_ignore[2];  // NOTE is_ignore[i] ignores both send and recv on i-th
                        // side
    {  // determine if communication is needed on each side of this direction
      is_ignore[0] =
          (sub_range[i_grid].far_begin() >= domain_range[i_grid].begin());
      is_ignore[1] =
          (sub_range[i_grid].far_end() <= domain_range[i_grid].end());
    }
    ...
    for (int lr_send = 0; lr_send < 2;
         ++lr_send) {  // 0 is to left, 1 is to right
      ...
      if (src && (*src == comm.rank())) {  // NOTE check on dest is not needed
        ...
        auto I_b_recv = I_b;
        I_b[i_grid] =
            Policy<T>::sendIb(sub_range[i_grid].size(), guard, lr_send);
        I_b_recv[i_grid] =
            Policy<T>::recvIb(sub_range[i_grid].size(), guard, lr_send);
...
struct copy_policy {
  ...
  static constexpr void from_recv_buf(T& v_data, const T& v_buf,
                                      bool) noexcept {
    v_data = v_buf;
  }

  static constexpr int sendIb(int bulk_dim, int guard,
                              bool is_send_to_right) noexcept {
    return is_send_to_right ? (bulk_dim - guard) : 0;
  }

  static constexpr int recvIb(int bulk_dim, int guard,
                              bool is_send_to_right) noexcept {

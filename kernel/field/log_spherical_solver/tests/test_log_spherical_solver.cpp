...
// FIXME not compiling now

// TEST_CASE("test curlE", "[field]") {
//   const int N = 128;
//   const apt::Grid<R, D> grid = {
//       {{0.0, std::log(30.0), N}, {0.0, std::acos(-1.0), N}}};
//   constexpr int g = 1;
...
//       E[r_]({i, j}) =
//           std::exp(-2 * grid[r_].absc(i, 0)) * (1 - 3 * cos_th * cos_th);
...
//     for (int j = 0; j < N + 1; ++j) {
//       for (int i = 0; i < N; ++i) {
//         R x = 2 * std::sin(2 * grid[th_].absc(j, 0)) *
//               std::exp(-2 * grid[r_].absc(i, 0));
//         R corr = delta * delta;
...
//   for (int i = 0; i < N; ++i) h.fr(i) = 1.0 / std::exp(2.0 * grid[r_].absc(i));
...
//     R delta = grid[r_].delta();
//     for (int j = 0; j < N;
//          ++j) {  // NOTE j < N, not N+1, because cell N is beyond high axis
//       for (int i = 0; i < N; ++i) {

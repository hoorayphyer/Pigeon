...
TEST_CASE("test curlE", "[.][field]") {
  ...
  std::optional<int> surf(5);
  std::optional<int> outer(N);
  std::optional<int> lo_axis(0);
  std::optional<int> hi_axis(N);

  const int b[D] = {0, 0};
  const int e[D] = {N, N+1};

  Diff_in_CurlE<R, D> diff_r{0, surf, outer, grid[r_].delta(), grid[th_].delta()};
  Diff_in_CurlE<R, D> diff_th{1, surf, outer, grid[r_].delta(), grid[th_].delta()};
  ...
  for (int j = range[th_].far_begin(); j < range[th_].far_end(); ++j) {
    for (int i = range[r_].far_begin(); i < range[r_].far_end(); ++i) {
      ...
      E[r_]({i, j}) = std::exp(-3 * grid[r_].absc(i, 0)) * (1 - 3 * cos_th * cos_th);
      E[th_]({i, j}) = -std::exp(-3 * grid[r_].absc(i, 0.5)) *
  ...
  SECTION("test dr  of \mathcal{E}_th") {
    ...
    for (int j = range[th_].begin(); j < range[th_].end()+1; ++j) {
      for (int i = *surf; i < *outer; ++i) {
        R x = 3 * std::sin(2*grid[th_].absc(j,0)) * std::exp(-3*grid[r_].absc(i,0));
        R corr = delta*delta;
  ...
  SECTION("test d theta of \mathcal{E}_r ") {
    ...
    for (int j = *lo_axis; j < *hi_axis + 1; ++j) {
      for (int i = range[r_].begin(); i < range[r_].end(); ++i) {
        R x = 3 * std::sin(2 * grid[th_].absc(j, 0)) *
              std::exp(-3 * grid[r_].absc(i, 0));
        R corr = delta*delta;
        CAPTURE(i, j, corr, x, output[phi_]({i, j}) - x);
        CHECK(output[phi_]({i, j}) == Approx(x).margin(corr));
      }
...
TEST_CASE("test curlB", "[field]") {
  ...
  _helper<R,D> h(grid, range[0].far_begin(), range[0].far_end(), range[1].far_begin(), range[1].far_end() );

  Diff_in_CurlB<R, D> diffh_r{0, surf, outer, h, grid[r_].delta(), grid[th_].delta()};
  Diff_in_CurlB<R, D> diffh_th{1, surf, outer, h, grid[r_].delta(), grid[th_].delta()};
  ...
  SECTION("test d theta of \mathcal{B}_r ") {
    ...
    R delta = grid[th_].delta();
    for (int j = *lo_axis; j < *hi_axis; ++j) { // NOTE upper bound is not *hi_axis + 1
      for (int i = range[r_].begin(); i < range[r_].end(); ++i) {

...
class FieldBC_coordinate : public FieldBC {
 private:
  ...
  template <typename T>
  void ReflectArrayAtAxis(MultiArray<T>& array, const Grid& grid,
                          FieldType field_type, int comp,
                          bool ispos = true) const {
    ...
    int increm[2] = {DirIncrement(trans[0], grid.extent()),
                     DirIncrement(trans[1], grid.extent())};
    int increm_axis = DirIncrement(axisDir, grid.extent());
    //    int istart = isupper ? grid.dims[axisDir] - 2 * grid.guard[axisDir] -
    //    isupper * stag_axisDir
    //                           : grid.guard[axisDir] - stag_axisDir;
    int istart =
        isupper ? grid.dims[axisDir] - grid.indent[boundary] - stag_axisDir : 0;
    ...
    for (int k = 0; k < grid.dims[trans[0]]; k++) {
      for (int j = 0; j < grid.dims[trans[1]]; j++) {
        ...
        for (int i = istart; i < iend; i++) {
          ...
          int imirror =
              isupper ? 2 * grid.dims[axisDir] - 2 * grid.indent[boundary] - 1 -
                            i - stag_axisDir
          ...
          if (!ispos && i == imirror) {
            ...
            array[i * increm_axis + transIndex] =
                array[imirror * increm_axis + transIndex];

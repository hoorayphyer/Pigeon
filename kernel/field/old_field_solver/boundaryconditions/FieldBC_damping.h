...
class FieldBC_damping : public FieldBC {
 private:
  ...
  Scalar _absorb_rate;
  Grid _grid_damp;
  Index _shift;  // store the index in the original grid that corresponds to new
                 // grid's (0,0,0)
  VectorField<Scalar> _B_bg;  // initial background B
  VectorField<Scalar> _E_bg;

  inline void Damp(Scalar& f_target, const Scalar& f_bg,
                   const Scalar lambda) const {
    f_target = (f_target - f_bg) * lambda + f_bg;
  }

  inline Scalar damping_profile(Scalar x) { return x * x / 2.0; }

  void InitBackground(const VectorField<Scalar>& Efield_bg,
                      const VectorField<Scalar>& Bfield_bg) {
  ...
  //     throw std::runtime_error( "array size mismatch when reading in damping
  //     background at boundary " + std::to_string(_bpos) + "! Local has size "
  //     + std::to_string(size) + ", whereas snapshot has size" +
  //     std::to_string(size_snapshot) );
  //   }
  ...
  //   Logger::print_screen( Logger::gVerbosityLvl, "Rank", Logger::thisRank,
  //   "reads in damping background at boundary", std::to_string(_bpos) );

  // }

 public:
  template <typename DBPane>
  FieldBC_damping(BoundaryPosition bpos, const Grid& grid, const DBPane& pane,
                  const VectorField<Scalar>& Efield_bg,
                  const VectorField<Scalar>& Bfield_bg)
    ...
    if (isupper) {
      _grid_damp.lower[dir] =
          grid.pos(dir, grid.dims[dir] - grid.indent[bpos] - 1, 1);
  ...
  virtual void Apply(Scalar dt, VectorField<Scalar>& Efield,
    ...
    //     if ( C_dir >= _shift[dir] && C_dir < _shift[dir] +
    //     _grid_damp.dims[dir] ) {

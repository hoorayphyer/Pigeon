...
void Rotating_Monopole_LogSph(FBC& bdry) {
  bdry.B1 = [](Scalar r_log, Scalar, Scalar) -> Scalar {
    return 1.0 / std::exp(2 * r_log);
  };
  ...
  bdry.E2 = [](Scalar r_log, Scalar theta, Scalar) -> Scalar {
    return -std::exp(-r_log) * std::sin(theta);
  };
  bdry.E3 = [](Scalar, Scalar, Scalar) -> Scalar { return 0.0; };
}

void Rotating_Dipole_LogSph(FBC& bdry) {
  bdry.B1 = [](Scalar x, Scalar theta, Scalar) -> Scalar {
    return 2.0 * std::cos(theta) * std::exp(-3 * x);
  };
  bdry.B2 = [](Scalar x, Scalar theta, Scalar) -> Scalar {
    return std::sin(theta) * std::exp(-3 * x);
  };
  bdry.B3 = [](Scalar, Scalar, Scalar) -> Scalar { return 0.0; };
...
void RestoreJToRealSpace(VectorField<Scalar>& JField, const Grid& grid) {
  ...
  for (int comp = 0; comp < 3; ++comp) {
    ...
    switch (comp) {
      case 0:
        h_func = [](std::array<Scalar, 3> q) -> Scalar {
          return coord.h2(q[0], q[1], q[2]) * coord.h3(q[0], q[1], q[2]);
        };
        break;
      case 1:
        h_func = [](std::array<Scalar, 3> q) -> Scalar {
          return coord.h3(q[0], q[1], q[2]) * coord.h1(q[0], q[1], q[2]);
        };
        break;
      case 2:
        h_func = [](std::array<Scalar, 3> q) -> Scalar {
...
void OldSolve<R, DGrid, RJ>::Init(const mpi::CartComm& cart,
  ...
  // NOTE Currently FieldUpdater only lives on primaries, which permits grid to
  // be copied into Fields set up E, B, J
  {
  ...
  {  // NOTE this is setting up E_bg and B_bg for damping and store in Efield,
     // Bfield. IC for E and B are set up in operator(). This is to make resume
     // work
    Efield.assign(0.0);
    Bfield.assign(0.0);
    const auto& grid = fuparams.grid;
...
void OldSolve<R, DGrid, RJ>::operator()(Field<R, 3, DGrid>& E,
  ...
  // NOTE
  // due to different staggering systems, during conversion, only copy the bulk
  // and send guards cells For 0 <= i < dim_bulk, 1) for unstaggered components
  // (MIDWAY), indexing is field::Field[i] <-> VectorField[guard + i]; 2) for
  // staggered components (INSITU), indexing is field::Field[i] <->
  // VectorField[guard + i - 1];

  {  // convert from new to old
    const auto& fugrid = fuparams.grid;
    auto convert_from_new = [&](auto& old_f, const auto& new_f) {
      ...
      for (int c = 0; c < 3; ++c) {
        ...
        for (int j = 0; j < fugrid.reducedDim(1) + (o[1] == INSITU); ++j) {
          for (int i = 0; i < fugrid.reducedDim(0) + (o[0] == INSITU); ++i) {
            old_f(c, i + g[0] - (o[0] == INSITU), j + g[1] - (o[1] == INSITU)) =
                new_f[c]({beg_new[0] + i, beg_new[1] + j});
          }
        }
      }
      // TODOL there is no need to copy guard cell values to current right?
      // Because field solver doesn't use those values, instead they have
      // boundary conditions
    };
    if (!is_initialized) {
    ...
    // 2. the new code passes in Jmesh, so conversion to real space current is
    // needed
    auto preJ = _fourpi / _mu;
    for (int c = 0; c < 3; ++c)
      for (int i = 0; i < current.gridSize(); ++i) current.ptr(c)[i] *= preJ;
  ...
  {  // convert from old back to new. Note to multiply by _mu
    auto convert_to_new = [&](const auto& old_f, auto& new_f) {
      const auto& grid = fuparams.grid;
      ...
      for (int c = 0; c < 3; ++c) {
        ...
        for (int j = 0; j < grid.reducedDim(1) + (o[1] == INSITU); ++j) {
          for (int i = 0; i < grid.reducedDim(0) + (o[0] == INSITU); ++i) {
            new_f[c]({beg_new[0] + i, beg_new[1] + j}) = old_f(
                c, i + g[0] - (o[0] == INSITU), j + g[1] - (o[1] == INSITU));
          }
        }
      }
    };

...
TEMPLATE_TEST_CASE("Time field updater 2D",
                   "[mpi]"
                   // periodic
                   ,
                   (aio::IndexType<-1, -1>), (aio::IndexType<-2, -2>),
                   (aio::IndexType<-4, -4>)) {
  using Real = float;
  ...
  if (cart_opt) {
    ...
    apt::array<apt::pair<bool>, DGrid> is_at_boundary;
    {
      for (int i = 0; i < DGrid; ++i) {
        ...
        is_at_boundary[i][RGT] = (Cartesian_Partition[i] > 0 &&
                                  Cartesian_Partition[i] - 1 == coords[i]);
      }
    }
    ...
    for (int i = 0; i < DGrid; ++i)
      grid[i] =
          supergrid[i].divide(std::abs(Cartesian_Partition[i]), coords[i]);

    constexpr int guard = 1;
    constexpr auto omega_t = [](Real t) -> Real {
      return 0.2 * (std::min<Real>(t / 4.0, 1.0));
    };
    ...
    field::Updater<Real, DGrid, RealJ> fu(*cart_opt, grid, is_at_boundary,
                                          guard, omega_t, re_over_w_gyro_unitB);
    ...
    auto dur = t1.lapse();
    cart_opt->barrier();
    WARN("Timing field updater per timestep = "
         << dur.in_units_of("ms").val() << "/" << N << " = "
         << dur.in_units_of("ms").val() / N << dur.unit());
  }
  mpi::world.barrier();
}

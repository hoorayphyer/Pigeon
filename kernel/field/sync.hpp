...
namespace apt {
  struct Range;
  template < typename T, int D > struct array;
}
...
namespace field {
  ...
  // 2. field's range == domain_range, but sync is called within an action range that is a subset
  ...
  template < typename T, int DField, int DGrid >
  inline void copy_sync_guard_cells( Field<T, DField, DGrid>& field, const mpi::CartComm& comm ) {
    copy_sync_guard_cells(field, comm, field.mesh().range(), field.mesh().range() );
  }
  ...
  template < typename T, int DField, int DGrid >
  inline void merge_sync_guard_cells( Field<T, DField, DGrid>& field, const mpi::CartComm& comm ) {
    merge_sync_guard_cells(field, comm, field.mesh().range(), field.mesh().range() );
  }
}

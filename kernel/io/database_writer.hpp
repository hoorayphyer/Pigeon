...
namespace io {
  ...
  private:
    ...
    std::unique_ptr<silo::file_t>
        m_master{}; // only significant on cart.rank() == 0, use reference because
                  // silo library is sloppy about const-ness.
    ...
    void set_namescheme(std::string str_ts, int num_files) {
      ...
      {
        m_file_ns = delimiter + std::string("data/timestep") + str_ts + "/set%d.silo" + delimiter + "n%" + std::to_string(num_files);
      }

      {
        ...
        for ( int i = 0; i < topos.size(); ++i ) {
          m_block_ns_postfix += delimiter + std::string("(n%" + std::to_string(strides[i]) + ")/") + std::to_string(strides[i+1]);
        }
      }
    }
    ...
    void write_var_impl(std::string varname,
      ...
      for (int i = 0; i < DGrid; ++i)
        dims[i] = fcomp.mesh().range(i).full_size();
  ...
  public:
    DatabaseWriter(const std::optional<mpi::CartComm>& cart_opt, const std::string& prefix, int timestep, double dt, int num_files) : m_cart_opt(cart_opt) {
      char str_ts[10];
...

      {
        if ( m_cart_opt ) {
          ...
          m_pmpio.filename = prefix + "/data/timestep" + str_ts + "/set" + std::to_string(m_cart_opt->rank() % num_files ) + ".silo";
          ...
          fs::mpido(*m_cart_opt, [&]() {
            ...
            m_master.reset(new silo::file_t(
    ...
    void write_mesh(const field::Mesh<DGrid>& mesh, bool is_collinear_mesh, int mesh_ghost, int downsample_ratio, const apt::Grid<Real,DGrid>& grid) const {
      if (!m_cart_opt) return;
      ...
      */
      ...
      if ( is_collinear_mesh ) {
        ...
        m_pmpio( [&](auto& dbfile) {
          dbfile.put_mesh_noncollinear(m_meshname, coords, quadmesh_dims, DGrid, optlist_mesh);
        } );
      ...
      auto mesh_type = is_collinear_mesh ? silo::MeshType::Rect : silo::MeshType::Curv;
      if (m_master)
    ...
    void write_var(const std::string &name, const std::string& name_postfix, int field_dim,
                   const field::Field<RealDS, 3, DGrid> &fds) const {
      for (int comp = 0; comp < field_dim; ++comp) {
        auto varname = name;
        if (field_dim > 1)
          varname += std::to_string(comp + 1);
        if (!name_postfix.empty())
          varname += "_" + name_postfix;

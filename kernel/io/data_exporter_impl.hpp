...
apt::Grid<Tcoarse, DGrid> coarsen(const apt::Grid<Tfine, DGrid> &grid,
                                  int downsample_ratio) {
  apt::Grid<Tcoarse, DGrid> res;
  for (int i = 0; i < DGrid; ++i) {
    res[i] = apt::Grid1D<Tcoarse>(grid[i].lower(), grid[i].upper(),
                                  grid[i].dim() / downsample_ratio);
  }
  return res;
}
}  // namespace
...
void DataExporter<RealDS, DGrid, Real, S, RealJ>::export_data(
  ...
  db_writer.write_mesh<RealDS>(E.mesh(), m_is_collinear_mesh, m_mesh_ghost,
                               m_downsample_ratio, grid);

  const auto grid_ds = coarsen<RealDS>(grid, m_downsample_ratio);

  field::Field<RealDS, 3, DGrid> fds(
      apt::make_range({}, apt::dims(grid_ds), m_mesh_ghost));

  if (cart_opt) {
    for (const auto &fe : m_fexps) {
      fds = fe->action(m_downsample_ratio, cart_opt, grid, grid_ds,
                       m_mesh_ghost, E, B, J);
  ...
  for (auto sp : particles) {
    ...
    for (const auto &pe : m_pexps) {
      fds = pe->action(m_downsample_ratio, grid, grid_ds, m_mesh_ghost, prop,
                       particles[sp]);

      // FIXME reduce number of communication
      for (int i = 0; i < pe->num_comps(); ++i)
        ens.reduce_to_chief(mpi::by::SUM, fds[i].data().data(),
                            fds[i].data().size());
      if (cart_opt) {

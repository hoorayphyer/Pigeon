...
struct Simulator {
 private:
  ...
  void taylor(apt::array<apt::Range, DGrid>& a) {
    ...
    for (int i = 0; i < DGrid; ++i) {
      auto [b_new, e_new] = f(a[i].begin(), a[i].end(), _supergrid[i], _grid[i],
                              pic::periodic[i]);
      a[i].begin() = b_new;
      a[i].end() = e_new;
    }
  ...
  void update_parts(const dye::Ensemble<DGrid>& ens) {
    for (int i = 0; i < DGrid; ++i) {
      _grid[i] =
          _supergrid[i].divide(ens.cart_topos[i].dim(), ens.cart_coords[i]);
    }
    ...
    {  // set particle actions
      // auto p_actions =
      // particle::set_up_particle_actions<DGrid,R,S,ShapeF,RJ>();
 ...
 public:
  Simulator(const apt::Grid<R, DGrid>& supergrid,
            const std::optional<mpi::CartComm>& cart_opt,
            const particle::map<particle::Properties>& props)
    ...
    {  // initialize fields. Idles also do this for the sake of loading
       // checkpoint, so pic::dims is used instead of _ens_opt or _cart_opt
    ...
    for (auto sp : _properties) _particles.insert(sp, particle::array<R, S>());
  ...
  int load_initial_condition(std::optional<std::string> checkpoint_dir) {
    ...
    if (checkpoint_dir) {
      init_ts = ckpt::load_checkpoint(*checkpoint_dir, _ens_opt, _cart_opt, _E,
                                      _B, _particles, _properties);
      if (_ens_opt) {
  ...
  // NOTE we choose to do particle update before field update so that in saving
  // snapshots we don't need to save _J
...
#define START [&]()
    ...
    if (_ens_opt) {
      ...
      TIMING(
          "BroadcastEB", START {
            ...
            for (int i = 0; i < 3; ++i)
              ens.intra.broadcast(ens.chief, _E[i].data().data(),
                                  _E[i].data().size());
            for (int i = 0; i < 3; ++i)
              ens.intra.broadcast(ens.chief, _B[i].data().data(),
                                  _B[i].data().size());
          });
      ...
      if (stamp && _particles.size() != 0) {
        ...
        for (auto sp : _particles)
          lgr::file % "\t" << _properties[sp].name << " = "
                           << _particles[sp].size() << std::endl;
      }
      ...
      for (int i = 0; i < _ptc_actions.size(); ++i) {
        ...
        TIMING(
            " -- " + _ptc_actions[i]->name(), START {
              auto& act = *_ptc_actions[i];
              act(_particles, _J, &_ptc_buffer, _properties, _E, _B, _grid,
                  &ens, dt, timestep, _rng);
            });
      }
      ...
      if (export_plan.is_do(timestep)) {
        TIMING(
            "ExportData", START {
              export_prior_hook(_particles, _properties, _E, _B, _J, _grid,
                                _cart_opt, ens, dt, timestep);
        ...
        TIMING(
            "Statistics", START {
              pic::check_vitals(pic::this_run_dir + "/vitals.txt",
                                timestep * dt, ens, _cart_opt, _properties,
                                _particles, RTD::data().N_scat);
            });
      }
    ...
    if (load_balance_plan.is_do(timestep)) {
      TIMING(
          "DynamicLoadBalance", START {
            ...
            dynamic_load_balance(_particles, _ens_opt, _cart_opt,
    ...
    static ckpt::Autosave
        autosave;  // significant only on mpi::world.rank() == 0
    if (checkpoint_plan.is_do(timestep)) {
      TIMING(
          "SaveCheckpoint", START {
            auto dir = ckpt::save_checkpoint(
                this_run_dir, checkpoint_plan.num_files, _ens_opt, timestep, _E,
                _B, _particles, _properties);
            if (mpi::world.rank() == 0) {
              auto obsolete_ckpt = autosave.add_checkpoint(
                  dir, checkpoint_plan.max_num_checkpoints);
      ...
      TIMING(
          "SaveTracing", START {
            auto dir = pic::Traman::save_tracing(
                this_run_dir, save_tracing_plan.num_files, _ens_opt, timestep,
                _particles);
          });
    }

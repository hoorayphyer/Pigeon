...
std::optional<mpi::CartComm> make_cart(
    const apt::array<int, pic::DGrid>& dims,
    const apt::array<bool, pic::DGrid> periodic) {
  std::optional<mpi::CartComm> cart_opt;
  int nprmy = 1;
  for (auto i : dims) nprmy *= i;
...
int main(int argc, char** argv) {
  ...
  if (cli_args.config_file) {
    try {
      ...
      std::cerr << "Error parsing file '"sv << *err.source().path << "':\n"sv
    ...
    if (resume_dir) {
      if (!fs::exists(*resume_dir)) {
        ...
        std::cout << "ERROR : Invalid resume directory : " << *resume_dir
                  << std::endl;
      } else if (resume_dir->find("checkpoints/timestep") ==
                 std::string::npos) {
  ...
  {  // use block to force destruction of potential mpi communicators before
     // mpi::finalize
    ...
    pic::Simulator<pic::DGrid, pic::real_t, particle::Specs, pic::ShapeF,
                   pic::real_j_t>
    ...
    for (int ts = init_timestep; ts < init_timestep + pic::total_timesteps;
         ++ts) {

...
namespace field {
  ...
  constexpr T diff_zero( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
    return 0.0;
  }
}
...
namespace field {
  ...
  constexpr T diff( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
    using Metric = mani::LogSphericalCoordSys;
    ...
    if constexpr ( INSITU == f_ofs ) // f being of Etype
  ...
  constexpr T (*Diff( int Fcomp, int Icoord )) (const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s)  noexcept {
    if ( 0 == Fcomp ) {
  ...
  template < int DGrid, typename T, int Fcomp, int I, offset_t f_ofs >
  constexpr T diff_1sided_from_right( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
    using Metric = mani::LogSphericalCoordSys;
    static_assert( DGrid > 1 );
    static_assert( I >= 0 && I < 3 );
    if constexpr ( I >= DGrid ) return 0.0;
    if constexpr ( INSITU == f_ofs ) // f being of Etype
                   return diff<DGrid,T,Fcomp,I,f_ofs>(f,lnr,theta,g,s);
  ...
  template < int DGrid, typename T, int Fcomp, int I, offset_t f_ofs >
  constexpr T diff_1sided_from_left( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
    using Metric = mani::LogSphericalCoordSys;
    static_assert( DGrid > 1 );
    static_assert( I >= 0 && I < 3 );
    if constexpr ( I >= DGrid ) return 0.0;
    if constexpr ( INSITU == f_ofs ) // f being of Etype
      return diff<DGrid,T,f_ofs,I>(f,lnr,theta,g,s);
    else { // f being of Btype
  ...
  constexpr T diff_axis_Ephi_theta( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
  ...
  template < int DGrid, typename T, int Fcomp, int I, offset_t f_ofs >
  constexpr T diff_beyond_hi_axis( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
  ...
  template < int DGrid, typename T, int Fcomp, int I, offset_t f_ofs >
  constexpr T diff_1sided_from_right_beyond_hi_axis( const T& f, T lnr, T theta, const mani::Grid<T,DGrid>& g, const apt::Index<DGrid>& s ) noexcept {
    constexpr bool IsAxissymmetric = ( Fcomp + I == 3 );
    return ( IsAxissymmetric ? 1 : -1 ) * diff_1sided_from_right<DGrid,T,Fcomp,I,f_ofs>( *(&f-s[1]), lnr, theta - g[1].delta(), g, s );
  }

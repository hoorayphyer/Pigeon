...
template <>
std::unordered_map<species, std::size_t> TracingManager::parse_tracing(
  ...
  for (auto sp : particles) {
    ...
    for (const auto& p : particles[sp]) {
      ...
      if (wr < comm.size()) counts[wr] = std::max(counts[wr], sn);
    }
...
template <>
std::string TracingManager::save_tracing(
  ...
  pmpio([&](auto& dbfile) {
    ...
    {  // write process specific data
      ...
      dbfile.write("r", r);
      {
        ...
        // NOTE: explicitly write load 1) to signal that this species in
        // principle can exist and 2) because in Silo array length has type int
        // so inferring from it might in some extreme case is wrong and hard to
        // debug

...
SCENARIO("Test Annihilation", "[particle][mpi]") {
  ...
  if (rwld_opt) {
    ...
    auto init_ptcs = [&grid](auto& ptcs, const auto frac, const auto Npc,
                             const auto rel) {
    ...
    auto get_Npc_el = [](bool is_odd_rank) { return is_odd_rank ? 2 : 10; };
    auto get_Npc_po = [](bool is_odd_rank) { return is_odd_rank ? 6 : 1; };
    ...
    {  // test
      auto num_e = count(el);
      auto num_p = count(po);
      rwld_opt->reduce<true>(mpi::by::SUM, 0, num_e[0].data().data(),
                             num_e[0].data().size());
      rwld_opt->reduce<true>(mpi::by::SUM, 0, num_p[0].data().data(),
                             num_p[0].data().size());

      for (int c = 0; c < 3; ++c)
        rwld_opt->reduce<true>(mpi::by::SUM, 0, J[c].data().data(),
                               J[c].data().size());
      ...
      rwld_opt->reduce<true>(mpi::by::SUM, 0, num_e_nofr[0].data().data(),
                             num_e_nofr[0].data().size());
      rwld_opt->reduce<true>(mpi::by::SUM, 0, num_p_nofr[0].data().data(),
                             num_p_nofr[0].data().size());

      if (rwld_opt->rank() == 0) {
        int size = rwld_opt->size();
        int num_odd = size / 2;
        int num_even = size - num_odd;
        REQUIRE(Npc_tot_el == Approx(frac_el * (get_Npc_el(true) * num_odd +
                                                get_Npc_el(false) * num_even)));
        REQUIRE(Npc_tot_po == Approx(frac_po * (get_Npc_po(true) * num_odd +
                                                get_Npc_po(false) * num_even)));
        ...
        {  // test J
          Vec_t J_bulk;
          {
            auto Jtmp = J;
            Jtmp.reset();
            int I[2] = {mesh.range(0).full_size() / 2,
                        mesh.range(1).full_size() /
                            2};  // get the center cell. Use full_size because
                                 // q_std requires so
            apt::array<R, 3> q1_std{I[0] + 0.5, I[1] + 0.5, 0.5};
            apt::array<R, 3> q0_std{I[0] + rel_el[0], I[1] + rel_el[1],
                                    q1_std[2]};
            ...
            msh::deposit(Jtmp, R(1.0) / dt, ShapeF(), q0_std, q1_std);
            for (int c = 0; c < 3; ++c)
              J_bulk[c] = std::accumulate(Jtmp[c].data().begin(),
                                          Jtmp[c].data().end(), R(0));
            J_bulk *= Nannih_pc;
          }

          for (int c = 0; c < 3; ++c) {
            for (int j = mesh.range(1).margin(0);
                 j < mesh.range(1).size() - mesh.range(1).margin(1); ++j) {
              for (int i = mesh.range(0).margin(0);
                   i < mesh.range(0).size() - mesh.range(0).margin(1); ++i) {
                CAPTURE(c, i, j);

...
struct Particle
    : public PtcExpression<Particle<T, Specs>, apt::Vec<T, Specs<T>::Dim>, T,
 ...
 public:
  ...
  Particle(const apt::VecExpression<E1>& q, const apt::VecExpression<E2>& p,
           T frac, const Attrs&... attrs) noexcept
  ...
  Particle(const vec_type& q, const vec_type& p, T frac,
           const Attrs&... attrs) noexcept
      : _q(q), _p(p), _frac(frac) {
    this->set(flag::exist);
    // NOTE need "this" before set because of dependent base lookup
    if constexpr (sizeof...(Attrs) > 0) this->set(attrs...);
  ...
  constexpr void swap(Particle& other) noexcept {
    apt::foreach<0, Specs<T>::Dim>([](auto& a, auto& b) { std::swap(a, b); },
                                   q(), other.q());
    apt::foreach<0, Specs<T>::Dim>([](auto& a, auto& b) { std::swap(a, b); },
                                   p(), other.p());

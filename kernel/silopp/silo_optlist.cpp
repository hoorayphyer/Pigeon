...
namespace silo {
  OptVal::OptVal( Opt id ) {
    ...
    case Val_t::FLOAT : _numeric = (float)0; break;
    case Val_t::DOUBLE : _numeric = (double)0; break;
    case Val_t::INT3 :
...
namespace silo {
  ...
  OptList::OptList( const OptList& other )
    : std::unordered_map<Opt,OptVal>( static_cast<const std::unordered_map<Opt,OptVal>&>(other) ) {
    _p.reset(DBMakeOptlist(3));
  }
  ...
  OptList::operator DBoptlist* () const {
    ...
    for ( auto& [id, val] : *this ) {
      ...
      if ( val_type == Val_t::INT3 ) {
        ...
        std::visit( [opt=raw_opt, ptr = _p.get()](auto& v)
                    { DBAddOption(ptr, opt, (void*)&v); }, val._numeric );
      }
    }

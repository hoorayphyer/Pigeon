...
namespace debug {
// compare two sets of particles which may or may not be distributed over
// processes modulo empty particles.
template <typename T, template <typename> class S>
std::array<std::size_t, 5> compare_particles(particle::array<T, S>&& ptcs1,
                                             particle::array<T, S>&& ptcs2,
                                             const mpi::Comm& comm) {
  using namespace particle;
  auto is_same = [](const auto& ptc1, const auto& ptc2) {
  ...
  auto count_ptcs = [](const auto& ptcs) {
    std::size_t num = 0;
  ...
  do {
    ...
    // pass ptcs1 to next process for further cancellation
    {
      ...
      for (int i = 0; i < count_s; ++i) buf_s[i] = std::move(ptcs1[i]);

      std::vector<mpi::Request> reqs(2);
      reqs[0] = comm.Isend((r + 1) % size, 147, buf_s.data(), buf_s.size());

      std::size_t count_r =
          comm.probe((r + size - 1) % size, 147, buf_r.data());
      buf_r.resize(count_r);
      reqs[1] = comm.Irecv((r + size - 1) % size, 147, buf_r.data(), count_r);
      mpi::waitall(reqs);

      for (int i = 0; i < count_r; ++i) ptcs1[i] = std::move(buf_r[i]);
    }

  } while (--n);

  return {num_ptcs1_before, num_ptcs2_before, count_matched, count_ptcs(ptcs1),
          count_ptcs(ptcs2)};
}
}  // namespace debug

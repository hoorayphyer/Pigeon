...
class ArrayProduct {
 ...
 public:
  ...
  public:
      ...
      typedef typename std::conditional<isConst, const data_type&, data_type&>::type data_ref_type;
      typedef typename std::conditional<isConst, const data_type*, data_type*>::type data_ptr_type;
      typedef typename std::conditional<isConst, const arrprd_type&, arrprd_type&>::type product_ref_type;
      typedef typename std::conditional<isConst, const arrprd_type*, arrprd_type*>::type product_ptr_type;

      /// Constructors and destructors
      const_nonconst_iterator( product_ref_type arrprd, index_type pos)// index ref for pos?????
  ...
  ArrayProduct(const T1& a1, const T2& a2) {
    if (sizeof(T1) > 2 * sizeof(T1*)) _array1 = &a1;
    else _array1 = new T1(a1);

    if (sizeof(T2) > 2 * sizeof(T2*)) _array2 = &a2;
    else _array2 = new T2(a2);
  }
  ArrayProduct(const self_type& other) {
    if (sizeof(T1) > 2 * sizeof(T1*)) _array1 = other._array1;
    else _array1 = new T1(*(other._array1));

    if (sizeof(T2) > 2 * sizeof(T2*)) _array2 = other._array2;
    else _array2 = new T2(*(other._array2));
  }
...
ArrayProduct<Array, ArrayProduct<T1, T2> > operator*(const Array& left,
                                               const ArrayProduct<T1, T2>& right) {
  return ArrayProduct<Array, ArrayProduct<T1, T2> >(left, right);
}

template <typename Array, typename T1, typename T2>
ArrayProduct<ArrayProduct<T1, T2>, Array> operator*(const ArrayProduct<T1, T2>& left,
                                              const Array& right) {

...
namespace apt {
  ...
  template < int D >
  constexpr array<Range,D> make_range( const array<int,D>& b, const array<int,D>& e, const array<pair<int>,D>& mg ) noexcept {
    array<Range,D> res;
  ...
  template < int D >
  constexpr array<Range,D> make_range( const array<int,D>& b, const array<int,D>& e, int mg ) noexcept {
    array<Range,D> res;
...
namespace apt::range {
  ...
  // setters
  template < int D >
  constexpr void set_begin( array<Range,D>& range, const array<int,D>& beg ) noexcept {
    for ( int i = 0; i < D; ++i ) range[i].begin() = beg[i];
  }

  template < int D >
  constexpr void set_end( array<Range,D>& range, const array<int,D>& end ) noexcept {
    for ( int i = 0; i < D; ++i ) range[i].end() = end[i];
  }

  template < int D >
  constexpr void set_margin( array<Range,D>& range, const array<pair<int>,D>& margin ) noexcept {
  ...
  template < int D >
  constexpr const int& begin( const array<Range,D>& range, int i ) noexcept { return range[i].begin(); }
  template < int D >
  constexpr int& begin( array<Range,D>& range, int i ) noexcept { return range[i].begin(); }

  template < int D >
  constexpr const int& end( const array<Range,D>& range, int i ) noexcept { return range[i].end(); }
  template < int D >
  constexpr int& end( array<Range,D>& range, int i ) noexcept { return range[i].end(); }

  template < int D >
  constexpr const pair<int>& margin( const array<Range,D>& range, int i ) noexcept { return range[i].margin(); }
  template < int D >
  constexpr pair<int>& margin( array<Range,D>& range, int i ) noexcept { return range[i].margin(); }

  template < int D >
  constexpr int size( const array<Range,D>& range, int i ) noexcept { return range[i].size(); }

  template < int D >
  constexpr int far_begin( const array<Range,D>& range, int i ) noexcept { return range[i].far_begin(); }

  template < int D >
  constexpr int far_end( const array<Range,D>& range, int i ) noexcept { return range[i].far_end(); }

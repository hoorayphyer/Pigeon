...
struct FCompWise_Vec_Vec
    : public VecExpression<FCompWise_Vec_Vec<Op, E1, E2, T>, T> {
 private:
  const E1& _e1;
  const E2& _e2;
 ...
 public:
  ...
  constexpr T operator[](int i) const noexcept {
    if constexpr (Op == BinaryOps::ADD)
      return _e1[i] + _e2[i];
    else if (Op == BinaryOps::SUB)
      return _e1[i] - _e2[i];
    else if (Op == BinaryOps::MUL)
      return _e1[i] * _e2[i];
    else if (Op == BinaryOps::DIV)
      return _e1[i] / _e2[i];
  }
};
...
struct FCompWise_Vec_Sca
    : public VecExpression<FCompWise_Vec_Sca<Op, E, Real, T>, T> {
 private:
  const E& _e;
  const Real& _t;
...
#define vec_def_op(_OP_, _OP_NAME_)                                         \
  ...
  constexpr auto operator _OP_(const apt::VecExpression<E1>& e1,            \
    ...
    return apt::FCompWise_Vec_Vec<apt::BinaryOps::_OP_NAME_, E1, E2>(       \
        static_cast<const E1&>(e1), static_cast<const E2&>(e2));            \
  }                                                                         \
                                                                            \
  template <typename E, typename Real>                                      \
  constexpr std::enable_if_t<                                               \
      std::is_arithmetic_v<Real>,                                           \
      apt::FCompWise_Vec_Sca<apt::BinaryOps::_OP_NAME_, E, Real> >          \
  operator _OP_(const apt::VecExpression<E>& e, const Real& t) noexcept {   \
  ...
  constexpr std::enable_if_t<                                               \
      std::is_arithmetic_v<Real>,                                           \
      apt::FCompWise_Vec_Sca<apt::BinaryOps::_OP_NAME_, E, Real> >          \
  operator _OP_(const Real& t, const apt::VecExpression<E>& e) noexcept {   \
    return {static_cast<const E&>(e), t};                                   \
  }
...
struct VecNegative : public VecExpression<VecNegative<E, T>, T> {
 ...
 public:
  ...
  constexpr T operator[](int i) const noexcept { return -(_e[i]); }
};
}  // namespace apt

namespace {
template <typename E>
constexpr apt::VecNegative<E> operator-(
    const apt::VecExpression<E>& vec) noexcept {
...
struct VecCross : public VecExpression<VecCross<E1, E2, T>, T> {
 ...
 public:
  ...
  constexpr VecCross(const E1& e1, const E2& e2) noexcept : _e1(e1), _e2(e2) {}

  constexpr T operator[](int i) const noexcept {
    return _e1[(i + 1) % 3] * _e2[(i + 2) % 3] -
           _e1[(i + 2) % 3] * _e2[(i + 1) % 3];
  }
};
...
constexpr auto dot(const VecExpression<E1>& v1,
                   const VecExpression<E2>& v2) noexcept {
  constexpr int END = ndim_v<E1>;
  if constexpr (I == END)
    return 0;
  else
    return v1[I] * v2[I] + dot<E1, E2, I + 1>(v1, v2);
}
...
constexpr auto abs(const VecExpression<E>& vec) noexcept {
  if constexpr (ndim_v<E> == 1)
    return std::abs(vec[0]);
  else
    return std::sqrt(sqabs(vec));
}

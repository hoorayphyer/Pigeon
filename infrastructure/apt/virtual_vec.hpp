...
struct ref_tuple_sequence {
  ...
  using type = decltype(std::tuple_cat(
      ...
      std::declval<typename ref_tuple_sequence<T, N - 1>::type>()));
};
...
struct vVec : public VecExpression<vVec<T, N>, T>,
 ...
 public:
  ...
  template <typename... U>
  constexpr vVec(U&... u) noexcept : _v(u...) {
    static_assert(sizeof...(U) == NDim);
  };

  constexpr vVec(array<T, N>& arr) noexcept
      : vVec(arr, std::make_index_sequence<N>{}) {}

  constexpr vVec(tuple_type&& tpl) noexcept : _v(std::move(tpl)){};

...
// NOTE MPI_Group being an ordered set seems to mean that ( group_rank, process
// ) are stored as elements and ordered by group_rank. The mapping is solely
// determined by the order of processes user passed in when constructing the
// group.
struct Group : apt::Handle<MPI_Group, group_free, group_empty> {
  ...
  std::vector<int> translate(const std::vector<int>& ranks,
...
struct Comm : public apt::Handle<MPI_Comm, comm_free, comm_null>,
  ...
  Comm() = default;
  // Comm ( const Comm& super_comm, const Group& sub_group ); // TODO what if
  // sub_group is empty?
...
struct CartComm : public Comm {
 ...
 public:
  ...
  std::optional<CartComm> sub(
      std::vector<int> coords_bottom_left,
      std::vector<int>
          coords_top_right);  // [ coords_bottom_left, coords_top_right )
  ...
  inline std::vector<int> coords() const { return rank2coords(rank()); }

  std::tuple<std::vector<int>, std::vector<Topo>> coords_topos() const;

  // Sending in the `ith_dim` by `disp`, returns [ src_rank, dest_rank ]. If
  // disp = 0, both src_rank and dest_rank will be self
...
struct InterComm : public apt::Handle<MPI_Comm, comm_free, comm_null>,
  ...
  InterComm(const Comm& local_comm, int local_leader,
            const std::optional<Comm>& peer_comm, int remote_leader, int tag);

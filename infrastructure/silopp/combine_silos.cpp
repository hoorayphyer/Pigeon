    ...
    for (const auto fl : fs::directory_iterator(dir)) {
      ...
      if (timesteps_treated.find(ts) != timesteps_treated.end()) {
        ...
        for (int i = 1; i <= num_obj; i += 2) {  // only odd i need be modified
          char name[20];
          snprintf(name, 10, "#%06d",
                   i);  // null terminator is automatically appended
          ...
          {  // retrieve stored file_ns
            ...
            H5Dread(did, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    (void*)(tmp.get()));  // it is null terminated
          ...
          {  // first create a new dataset
            std::string file_ns_new =
                file_ns[0] + dir.substr(2) + "/" + file_ns.substr(1);
            hsize_t len = file_ns_new.size() + 1;  // +1 for null terminator
            hid_t dataspace = H5Screate_simple(
                1, &len, NULL);  // second arg is the current dimensions

            hid_t did = H5Dcreate(group_id, name, datatype, dataspace,
                                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

            // then write the new file_ns
            H5Dwrite(did, H5T_STD_U8LE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     (void*)(file_ns_new.c_str()));

...
template <typename file_t>
void Reader<file_t>::readslice(std::string varname,
                               const std::vector<Slice>& slice, void* var) {
  ...
  if (!has_zero) {
    DBReadVarSlice(_dbfile(), varname.c_str(), offset.data(), length.data(),
                   stride.data(), ndims, var);
  }
}

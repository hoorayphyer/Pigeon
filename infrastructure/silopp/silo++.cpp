...
file_t open(std::string filename, Mode mode) {
  ...
  if (Mode::Read == mode)
    dbfile.reset(
        new DBfileHandle(DBOpen(filename.c_str(), traits::filetype, DB_READ)));
  else {
    // try open with DB_APPEND, if failed, do DBCreate
    // TODOL edge case: the file exists by accident so is garbage, but we need a
    // new file Temporarily disable error string to be printed
    ...
    if (NULL == db) {
      db = DBCreate(filename.c_str(), traits::create_mode,
                    traits::create_target, NULL, traits::filetype);
    }

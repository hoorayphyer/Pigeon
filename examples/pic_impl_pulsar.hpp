...
constexpr int myguard = std::max(
    ::field::LogSphericalSolver<real_t, DGrid, real_j_t>::min_guard(
        field_op_inv_precision),
...
auto set_up_field_actions() {
  ...
  struct RotatingConductor : public FieldAction {
   ...
   public:
    ...
    virtual RotatingConductor* Clone() const {
      return new RotatingConductor(*this);
    }

    virtual void operator()(Field<3>& E, Field<3>& B, JField&, const Grid& grid,
                            const mpi::CartComm&, int timestep,
                            real_t dt) const override {
      auto impose = [this, &grid, time = timestep * dt](
      ...
      for (int C = 0; C < 3; ++C) impose(E, _E_cond, C);
      for (int C = 0; C < 3; ++C) impose(B, _B_cond, C);
    }
  } fu_cond;
  {
    auto& fu = fu_cond;
  ...
  struct DampingLayer : public FieldAction {
   ...
   public:
    ...
    virtual void operator()(Field<3>& E, Field<3>& B, JField&, const Grid& grid,
                            const mpi::CartComm&, int timestep,
                            real_t dt) const override {
      auto impose = [this, &grid, dt](auto& F, const auto& F_bg, int comp) {
        const auto& ofs = F[comp].offset();
        const auto& m = F.mesh();
      ...
      for (int C = 0; C < 3; ++C) impose(E, _E_bg, C);
      for (int C = 0; C < 3; ++C) impose(B, _B_bg, C);
    }
  } fu_damp;
  {
    auto& fu = fu_damp;
    ...
    auto profile = [](real_t lnr) -> real_t {
      static real_t r_damp_b =
          std::exp(pic::supergrid[0].upper()) - damping_thickness;
      lnr = (std::exp(lnr) - r_damp_b) / damping_thickness;
      return 0.5 * lnr * lnr;
    };

    const int damping_begin_index = supergrid[0].csba(
        std::log(std::exp(supergrid[0].upper()) - damping_thickness));
...
auto set_up_particle_actions() {
  ...
  } escape;
  {
    ...
    const int escape_begin_index = supergrid[0].csba(
        std::log(std::exp(supergrid[0].upper()) -
...
using DataExporter_t =
    io::DataExporter<real_export_t, DGrid, real_t, particle::Specs, real_j_t>;
...
std::vector<DataExporter_t> set_up_data_exporters() {
  auto add_common_exportees = [](auto& exporter) {
    ...
    exporter
        ...
        .add_exportee(
            new FA("PairCreationRate", 1, pair_creation_rate, nullptr))
        .add_exportee(
      ...
      for (auto sp : *(RTD::data().Jsp)) {
        switch (sp) {
          case species::electron:
            exporter.add_exportee(new FA("Je", 3,
                                         J_by_species<species::electron>,
                                         average_and_divide_flux_by_area));
            break;
          case species::positron:
            exporter.add_exportee(new FA("Jp", 3,
                                         J_by_species<species::positron>,
                                         average_and_divide_flux_by_area));
            break;
          case species::ion:
            exporter.add_exportee(new FA("Ji", 3, J_by_species<species::ion>,
                                         average_and_divide_flux_by_area));
            break;
          default:;
        }
    ...
    exporter.add_exportee(new PA("Num", 1, ptc_num, nullptr))

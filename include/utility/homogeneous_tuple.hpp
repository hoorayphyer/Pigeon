#ifndef  _HOMOGENEOUS_TUPLE_HPP_
#define  _HOMOGENEOUS_TUPLE_HPP_

#include <cmath>
#include "homogeneous_tuple_impl.hpp"
#include <utility>
#include <experimental/type_traits> // for is_detected

namespace hmt {

  // a tuple whose elements are of the same type, which can be a reference type.
  // RATIONALE what I need : 1. type check on all Args... 2. create correct type of homotuple using Args
  template < typename... Args >
  using tuple = impl::homotuple<Args...>;

  template < typename... Args >
  inline auto make_tuple ( Args&&... args ) {
    return impl::make_homotuple( std::forward<Args>(args)... );
  }

  template < typename... Args >
  inline auto tie ( Args&&... args ) noexcept {
    return impl::tie_to_homotuple( std::forward<Args>(args)... );
  }

  template < typename... Args >
  inline auto forward_as_tuple ( Args&&... args ) noexcept {
    return impl::forward_as_homotuple( std::forward<Args>(args)... );
  }
}

namespace hmt {
  // template < std::size_t BEG, std::size_t END, typename Func, typename... T, std::size_t... N >
  // constexpr auto for_I_in( vtuple<T,N>... args, Func f ) {
...
//   // TODO check return tuple of void, if it is OK
...
//   // template < std::size_t BEG, std::size_t END, std::size_t(*Next)(std::size_t) = [](std::size_t x){return x+1;}  >
//   // constexpr auto cfor() {
//   //   return [] ( auto& f ) {
//   //            for ( int i = 0; )
//   //          }

...
namespace apt {
  namespace impl {
    ...
    constexpr decltype(auto) apply_impl( const Func& f, Arg&& arg, std::integer_sequence<I...>  ) noexcept {
  ...
  constexpr decltype(auto) apply( const Func& f, Arg&& arg  ) noexcept {
    return impl::apply_impl( f, std::forward<Arg>(arg), std::make_integer_sequence<int, D>{} );
  }
}
